...
public class AuthorizedInterceptor  implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        if (!flag){
        	/** 1.获取session中的用户  */
        	Admin admin = (Admin) request.getSession().getAttribute(HrmConstants.ADMIN_SESSION);
        	Manager manager = (Manager) request.getSession().getAttribute(HrmConstants.MANAGER_SESSION);
        	Employee employee = (Employee) request.getSession().getAttribute(HrmConstants.EMPLOYEE_SESSION);
        	/** 2.判断用户是否已经登录 */
