...
public class EmployeeController {
	...
	@RequestMapping(value="/employee/selectMyEmployee")
	 public String selectMyEmployees(Integer pageIndex,
			 Integer job_id,Integer dept_id,
			 @ModelAttribute Employee employee,
			 HttpSession session,
			 Model model){
	...
	@RequestMapping(value="/myEmployee/selectEmployee")
	 public String selectMyEmployee(Integer pageIndex,
			 Integer job_id,Integer dept_id,
			 @ModelAttribute Employee employee,
			 HttpSession session,
			 Model model){
		
		Manager manager = (Manager)session.getAttribute(HrmConstants.MANAGER_SESSION);
	...
	@RequestMapping(value="/myEmployee/addEmployee")
	 public ModelAndView addMyEmployee(
			 String flag,
			 Integer job_id,Integer dept_id,
			 HttpSession session,
			 @ModelAttribute Employee employee,
		...
		if(flag.equals("1")){
			...
			mv.addObject("depts", depts);
			// 返回添加员工页面
			mv.setViewName("myEmployee/showAddEmployee");
		} else{
			// 判断是否有关联对象传递，如果有，创建关联对象
			this.genericAssociation(job_id, dept_id, employee);
			// 添加操作
			hrmService.addEmployee(employee);
			// 设置客户端跳转到查询请求
			mv.setViewName("redirect:/myEmployee/selectEmployee");
		}
...
//		mv.setViewName("forward:/employee/selectEmployee");
	 ...
	 * @param ModelAndView mv
	 * */
	@RequestMapping(value="/myEmployee/removeEmployee")
	 public ModelAndView removeMyEmployee(String ids,ModelAndView mv){
		// 分解id字符串
		String[] idArray = ids.split(",");
		for(String id : idArray){
			// 根据id删除员工
...
//		mv.setViewName("forward:/employee/selectEmployee");
	 ...
	 * @param ModelAndView mv
	 * */
	@RequestMapping(value="/myEmployee/updateEmployee")
	 public ModelAndView updateMyEmployee(
			 String flag,
			 Integer job_id,Integer dept_id,
			 @ModelAttribute Employee employee,
			 ModelAndView mv){
		if(flag.equals("1")){
			...
			mv.addObject("employee", target);
			// 返回修改员工页面
			mv.setViewName("myEmployee/showUpdateEmployee");
		}else{
			// 创建并封装关联对象
			this.genericAssociation(job_id, dept_id, employee);
			System.out.println("updateEmployee -->> " + employee);
	 ...
	 * */
	public void genericAssociation(Integer job_id,
